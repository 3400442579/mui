...
namespace An.Image.Gif.Encoder
{
    ...
    public class PixelUtil
    {
        ...
        private readonly SixLabors.ImageSharp.Image _source = null;
        ...
        {
            _source = SixLabors.ImageSharp.Image.Load(source);
        }
        ...
        public WriteableBitmap UnlockBitsAndCrop(Rect rect)
        {
            ...
            var result = new WriteableBitmap(rect.Width, rect.Height, _source.Metadata.HorizontalResolution, _source.Metadata.VerticalResolution, _source.Format, _source.Palette);
            result.Lock();
        ...
        public List<Color> GetAllPixels()
        {
            ...
            if (Depth == 32) //For 32 bpp get Red, Green, Blue and Alpha
            {
                for (var i = 0; i + 3 < Pixels.Length; i += 4)
                    list.Add( Color.FromArgb ( blue: Pixels[i],green: Pixels[i + 1], red: Pixels[i + 2], alpha: Pixels[i + 3] ));
            ...
            else if (Depth == 24) //For 24 bpp get Red, Green and Blue
            {
                for (var i = 0; i + 2 < Pixels.Length; i += 3)
                    list.Add(Color.FromArgb(blue: Pixels[i], green: Pixels[i + 1], red: Pixels[i + 2]));
