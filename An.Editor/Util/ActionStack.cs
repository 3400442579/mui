...
namespace An.Editor.Util
{
    ...
    public static class ActionStack
    {
        ...
        public static void SaveState(EditAction action, List<Frame> frames, List<int> positions)
        {
            if (!ShouldSaveState())
                return;

            var orderedPositions = positions.OrderBy(x => x).ToList();
            var savedFrames = new List<Frame>();
            ...
            switch (action)
            {
                case EditAction.Remove:
                    ...
                    foreach (var position in orderedPositions)
                    {
                        ...
                        savedFrames.Add(new Frame(savedFrame, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index));
                    }
                ...
                case EditAction.Properties:
                    ...
                    foreach (var position in orderedPositions)
                    {
                        ...
                        savedFrames.Add(new Frame(frame.Path, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index));
                    }
        ...
        public static void SaveState(EditAction action, List<Frame> frames, List<int> removeList, List<int> alterList)
        {
        ...
        public static void SaveState(EditAction action, int position, int quantity)
        {
            ...
            {
                Cause = action,
                Indexes = Enumerable.Range(position, quantity).ToList()
            });
        ...
        public static void SaveState(EditAction action, List<Frame> frames)
        {
        ...
        public static List<Frame> Undo(List<Frame> current, bool pushToRedo = true)
        {
            //Pop from Undo stack.
            ...
            if (pushToRedo)
            {
                ...
                switch (latestUndo.Cause)
                {
                    ...
                    case EditAction.ImageAndProperties:
                        ...
                        var savedFrames2 = new List<Frame>();
                        ...
                        foreach (var position in latestUndo.Indexes)
                        {
                            ...
                            savedFrames2.Add(new Frame(savedFrame, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index));
                        }
                    ...
                    case EditAction.Properties:
                        ...
                        redoStateChange.Frames = new List<Frame>(latestUndo.Frames);
                        redoStateChange.Indexes = new List<int>(latestUndo.Indexes);
                    ...
                    case EditAction.Reorder:
                        ...
                        redoStateChange.Frames = CopyList(current);
                    ...
                    case EditAction.RemoveAndAlter:
                        ...
                        redoStateChange.Frames = new List<Frame>(latestUndo.Frames).Skip(latestUndo.Indexes.Count).Take(latestUndo.Indexes2.Count).ToList();
                        redoStateChange.Indexes = new List<int>(latestUndo.Indexes);
                    ...
                    case EditAction.AddAndAlter: //Check.
                        ...
                        var savedFrames3 = new List<Frame>();
                        ...
                        foreach (var position in latestUndo.Indexes)
                        {
                            ...
                            savedFrames3.Add(new Frame(savedFrame, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index));
                        }
                        ...
                        {
                            var frame = current[position];
                            savedFrames3.Add(new Frame(frame.Path, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index));
                        }
            ...
            switch (latestUndo.Cause)
            {
                case EditAction.Remove:
                    ...
                    foreach (var index in latestUndo.Indexes)
                    {
                        ...
                        //Add to list.
                        current.Insert(index, new Frame(file, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index));
                ...
                case EditAction.ImageAndProperties:
                    ...
                    foreach (var frame in latestUndo.Frames)
                    {
                        ...
                        current[latestUndo.Indexes[alteredIndex2]] = new Frame(currentFrame.Path, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index); //Image location stays the same.
                ...
                case EditAction.Properties:
                    ...
                    foreach (var frame in latestUndo.Frames)
                    {
                        current[latestUndo.Indexes[alteredIndex]] = new Frame(frame.Path, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index);
                ...
                case EditAction.Reorder:
                    ...
                    current = CopyList(latestUndo.Frames);
                ...
                case EditAction.RemoveAndAlter:
                    ...
                    {
                        current[latestUndo.Indexes2[alteredIndex3]] = new Frame(frame.Path, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index);
        ...
        public static List<Frame> Redo(List<Frame> current)
        {
            //Pop from Redo stack.
            ...
            switch (latestRedo.Cause)
            {
                ...
                case EditAction.Properties:
                    ...
                    undoStateChange.Frames = new List<Frame>(latestRedo.Frames);
                    undoStateChange.Indexes = new List<int>(latestRedo.Indexes);
                ...
                case EditAction.Reorder:
                    ...
                    undoStateChange.Frames = CopyList(current);
                ...
                case EditAction.RemoveAndAlter:
                    ...
                    //Save only the altered frames.
                    undoStateChange.Frames = new List<Frame>(latestRedo.Frames).Skip(latestRedo.Indexes.Count).Take(latestRedo.Indexes2.Count).ToList();
                    undoStateChange.Indexes = new List<int>(latestRedo.Indexes);
            ...
            switch (latestRedo.Cause)
            {
                ...
                case EditAction.Properties:
                    ...
                    foreach (var frame in latestRedo.Frames)
                    {
                        current[latestRedo.Indexes[alteredIndex]] = new Frame(frame.Path, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index);
                ...
                case EditAction.Reorder:
                    ...
                    current = CopyList(latestRedo.Frames);
                ...
                case EditAction.RemoveAndAlter:
                    ...
                    {
                        current[latestRedo.Indexes2[alteredIndex3]] = new Frame(frame.Path, frame.Delay, frame.CurX, frame.CurY, frame.Clicked, frame.Keys, frame.Index);
        ...
        public static List<Frame> Reset(List<Frame> current)
        {
        ...
        {
            if (Config.All.HistoryLimit <= 0)
                return;

            if (UndoStack.Count <= Config.All.HistoryLimit)
                return;

            try
            {
                for (var i = Config.All.HistoryLimit; i < UndoStack.Count; i++)
                {
        ...
        {
            return Config.All.HistoryLimit > 0;
        }
        ...
        private static List<Frame> CopyList(List<Frame> target)
        {
            return new List<Frame>(target.Select(s => new Frame(s.Path, s.Delay, s.CurX, s.CurY, s.Clicked, s.Keys, s.Index)));
        }
    }
}