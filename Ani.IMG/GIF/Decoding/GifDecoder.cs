...
namespace Ani.IMG.GIF.Decoding
{
    public sealed class GifDecoder : IDisposable
    {
        ...
        internal static Dictionary<ulong, GifColor[]> GlobalColorTableCache
            = new Dictionary<ulong, GifColor[]>();
        ...
        public void RenderFrame(int fIndex, string outfile = "")
        {
            lock (_lockObj)
            {
                ...
                //avalonia·½Ê½
                //WriteableBitmap bitmap = new WriteableBitmap(new PixelSize(Header.Dimensions.Width, Header.Dimensions.Height), new Vector(96, 96), PixelFormat.Bgra8888);
        ...
        private void DrawFrame(GifFrame curFrame, Memory<byte> _frameIndexSpan)
        {
            ...
            var activeColorTable = GlobalColorTableCache[activeColorTableHash];
        ...
        private static ulong ProcessColorTable(ref Stream stream, byte[] rawBufSpan, int nColors)
        {
            ...
            GlobalColorTableCache.Add(tableHash, targ);
            ...
            switch (extType)
            {
                case ExtensionType.GRAPHICS_CONTROL:
                    ...
                    currentFrame.FrameDelay =
                        TimeSpan.FromMilliseconds(StreamExtensions.SpanToShort(tempBuf.AsSpan(1)) * 10);
                ...
                case ExtensionType.APPLICATION:
                    ...
                    if (blockHeader.SequenceEqual(NetscapeMagic.Span))
                    {
                        ...
                        var _iterationCount = StreamExtensions.SpanToShort(tempBuf.AsSpan(1));

                        Header.Iterations = _iterationCount;
                    }
                    else
                        str.SkipBlocks(tempBuf);
                    break;
