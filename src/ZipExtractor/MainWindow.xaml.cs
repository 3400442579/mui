...
namespace ZipExtractor
{
    ...
    public partial class MainWindow : Window
    {
                ...
                _backgroundWorker.DoWork += (o, eventArgs) =>
                {
                    foreach (var process in Process.GetProcesses())
                    {
                        try
                        {
                            ...
                            {
                                _logBuilder.AppendLine("等待退出应用程序...");

                                _backgroundWorker.ReportProgress(0, "等待退出应用程序...");
                                process.WaitForExit();
                            }
                        }
                    ...
                    for (var index = 0; index < dir.Count; index++)
                    {
                        ...
                        string currentFile = string.Format("提取文件 {0}", entry.FilenameInZip);
        ...
        private void Window_Closing(object sender, CancelEventArgs e)
        {
            ...
            _logBuilder.AppendLine();
            //MessageBox.Show(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ZipExtractor.log"));
