    ...
    public abstract class DpiAwareWindow : Window
    {
        ...
        private IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            if (msg == NativeMethods.WM_DPICHANGED)
            {
                ...
                SetCurrentValue(LeftProperty, ul.X);
                SetCurrentValue(TopProperty, ul.Y);
                UpdateWindowSize(hw.X, hw.Y);

                // Remember the current DPI settings.
                var oldDpiX = this.DpiInformation.MonitorDpiX;
                var oldDpiY = this.DpiInformation.MonitorDpiY;
                ...
                {
                    this.DpiInformation.UpdateMonitorDpi(dpiX, dpiY);
        ...
        private void UpdateLayoutTransform()
        {
            if (this.isPerMonitorDpiAware)
            {
                ...
                if (root != null)
                {
                    if (this.DpiInformation.ScaleX != 1 || this.DpiInformation.ScaleY != 1)
                    {
                        root.LayoutTransform = new ScaleTransform(this.DpiInformation.ScaleX, this.DpiInformation.ScaleY);
                    }
                    else
                    {
        ...
        protected void RefreshMonitorDpi()
        {
            ...
            var dpiVector = this.DpiInformation.UpdateMonitorDpi(xDpi, yDpi);
