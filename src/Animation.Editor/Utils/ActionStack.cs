...
namespace Animation.Editor.Utils
{
    ...
    public static class ActionStack
    {
        ...
        public static Fis Project { get; set; }
        ...
        /// <param name="frames">要处理的帧</param>
        /// <param name="positions">帧的index</param>
        public static void SaveState(EditAction action, List<Fi> frames, List<int> positions)
        {
            var orderedPositions = positions.OrderBy(x => x).ToList();
            var savedFrames = new List<Fi>();
            ...
            switch (action)
            {
                case EditAction.Remove:
                    ...
                    foreach (var frame in frames)
                    {
                        ...
                        savedFrames.Add(new Fi(savedFrame, frame.Delay, frame.Cursor,  frame.Keys,frame.Index));
                    }
                ...
                case EditAction.Properties:
                    ...
                    {
                        //var frame = frames[position];
                        savedFrames.Add(new Fi(frame.Path, frame.Delay, frame.Cursor,   frame.Keys, frame.Index));
                    }
        ...
        /// <param name="removeList">删除帧</param>
        /// <param name="alterList">修改帧参数，不影响图片的帧</param>
        public static void SaveState(EditAction action, List<Fi> frames, List<int> removeList, List<int> alterList)
        {
        ...
        public static void SaveState(EditAction action, List<Fi> frames)
        {
        ...
        public static List<Fi> Undo(List<Fi> current, bool pushToRedo = true)
        {
            //Pop from Undo stack.
                ...
                switch (latestUndo.Cause)
                {
                    ...
                    case EditAction.ImageAndProperties:
                        ...
                        foreach (var position in latestUndo.Indexes)
                        {
                            ...
                            savedFrames2.Add(new Fi(savedFrame, frame.Delay, frame.Cursor,  frame.Keys, frame.Index));
                        }
                    ...
                    case EditAction.Properties:
                        ...
                        redoStateChange.Frames = new List<Fi>(latestUndo.Frames);
                        redoStateChange.Indexes = new List<int>(latestUndo.Indexes);
                    ...
                    case EditAction.RemoveAndAlter:
                        ...
                        redoStateChange.Frames = new List<Fi>(latestUndo.Frames).Skip(latestUndo.Indexes.Count).Take(latestUndo.Indexes2.Count).ToList();
                        redoStateChange.Indexes = new List<int>(latestUndo.Indexes);
                    ...
                    case EditAction.AddAndAlter: //Check.
                        ...
                        foreach (var position in latestUndo.Indexes)
                        {
                            ...
                            savedFrames3.Add(new Fi(savedFrame, frame.Delay, frame.Cursor,  frame.Keys, frame.Index));
                        }
                        ...
                        {
                            var frame = current[position];
                            savedFrames3.Add(new Fi(frame.Path, frame.Delay, frame.Cursor,  frame.Keys, frame.Index));
                        }
            ...
            switch (latestUndo.Cause)
            {
                case EditAction.Remove:
                    ...
                    foreach (var index in latestUndo.Indexes)
                    {
                        ...
                        //Add to list.
                        current.Insert(index, new Fi(file, frame.Delay, frame.Cursor,  frame.Keys, frame.Index));
                ...
                case EditAction.ImageAndProperties:
                    ...
                    foreach (var frame in latestUndo.Frames)
                    {
                        ...
                        current[latestUndo.Indexes[alteredIndex2]] = new Fi(currentFrame.Path, frame.Delay, frame.Cursor,  frame.Keys, frame.Index); //Image location stays the same.
                ...
                case EditAction.Properties:
                    ...
                    foreach (var frame in latestUndo.Frames)
                    {
                        current[latestUndo.Indexes[alteredIndex]] = new Fi(frame.Path, frame.Delay, frame.Cursor,  frame.Keys, frame.Index);
                ...
                case EditAction.RemoveAndAlter:
                    ...
                    {
                        current[latestUndo.Indexes2[alteredIndex3]] = new Fi(frame.Path, frame.Delay, frame.Cursor,  frame.Keys, frame.Index);
        ...
        public static List<Fi> Redo(List<Fi> current)
        {
            //Pop from Redo stack.
            ...
            switch (latestRedo.Cause)
            {
                ...
                case EditAction.Properties:
                    ...
                    undoStateChange.Frames = new List<Fi>(latestRedo.Frames);
                    undoStateChange.Indexes = new List<int>(latestRedo.Indexes);
                ...
                case EditAction.RemoveAndAlter:
                    ...
                    //Save only the altered frames.
                    undoStateChange.Frames = new List<Fi>(latestRedo.Frames).Skip(latestRedo.Indexes.Count).Take(latestRedo.Indexes2.Count).ToList();
                    undoStateChange.Indexes = new List<int>(latestRedo.Indexes);
            ...
            switch (latestRedo.Cause)
            {
                ...
                case EditAction.Properties:
                    ...
                    foreach (var frame in latestRedo.Frames)
                    {
                        current[latestRedo.Indexes[alteredIndex]] = new Fi(frame.Path, frame.Delay, frame.Cursor,  frame.Keys, frame.Index);
                ...
                case EditAction.RemoveAndAlter:
                    ...
                    {
                        current[latestRedo.Indexes2[alteredIndex3]] = new Fi(frame.Path, frame.Delay, frame.Cursor,  frame.Keys, frame.Index);
        ...
        public static List<Fi> Reset(List<Fi> current)
        {
        ...
        private static List<Fi> CopyList(this List<Fi> target)
        {
            return new List<Fi>(target.Select(s => new Fi(s.Path, s.Delay, s.Cursor, s.Keys, s.Index)));
        }
    }
}
